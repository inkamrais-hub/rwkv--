...
**s^τ 机制**起源于 RWKV-4（Peng 等，2023），其中在时间衰减参数上注入逐通道标量 τ，在最优 τ 下产生了惊人的 **−85% PPL** 改善。该机制利用了单层指数归一化步骤（`exp(QK^T) / Σ exp(...)`）：输入的微小乘性变化在输出端被指数级放大。（ps：不完全是，算是我对rwkv的一种探索性研究）
...
**单一架构范围。** τ 注入仅在 RWKV-7 上展示。尽管我们在 §6.5 中论证该方法学是模型无关的，在 Mamba、GLA 或 RetNet 上的经验验证将显著增强通用性声明。（在基于softmax的atth中，我们同时注入并微调了qwen3-1.7b模型，优化效果不错，但是在于计算资源实在没办法支持大规模微调，我们只能微调其2k stop/1ep。但是效果是显著的，在对比同等参数微调的softmax-atth，Top-1 集中度为0.425，下降43%。有效秩提升105%，值为5.08，cos相似度为0.804，详细数据我会在后续开源，现在本地太多了，又要同时推大规模微调，力不从心）
